...
public class WebUtil {
    ...
    public HttpServletRequest getRequest() {
        RequestAttributes requestAttributes = Optional.ofNullable(RequestContextHolder.getRequestAttributes()).orElseThrow(() -> new TipException("request is null"));
    ...
    /**
     * 获取websocket请求的 IP 地址
     *
     * @return {String}
     */
    public static String getSocketIp(HttpHeaders httpHeaders, ChannelHandlerContext ctx) {
        String ip = null;
        // 遍历 IP 头部名称数组
        for (String ipHeader : IP_HEADER_NAMES) {
            // 从 HttpServletRequest 中获取 IP 头部的值
            ip = httpHeaders.get(ipHeader);
            if (!IP_PREDICATE.test(ip)) {
                // 如果 IP 地址非空，即找到了有效的 IP 地址，则跳出循环
                break;
            }
